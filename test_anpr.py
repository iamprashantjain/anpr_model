...
if user_menu == "CV (CS)":
	...
	st.text("Note: Make Year will appear once Make & Model is Selected from drop downs.")


	col1,col2,col3,col4 = st.columns(4)
	col5,col6,col7,col8,col9 = st.columns(5)

	make = sorted(df['Make_Clean'].unique().tolist())
	make.insert(0,"Select Make")
	Make_Clean = col1.selectbox('Select Make',make,index = 0)
	
	model = sorted(df['Model_Clean'].loc[df['Make_Clean'] == Make_Clean].drop_duplicates().tolist())
	model.insert(0,"Select Model")
	Model_Clean = col2.selectbox('Select Model',model,index = 0)
	

	try:
		n1 = df['MAKE_YEAR'].loc[df['Model_Clean'] == Model_Clean].min()
		n2 = df['MAKE_YEAR'].loc[df['Model_Clean'] == Model_Clean].max()
		y1 = pd.to_datetime(n1, format='%Y')
		y2 = pd.to_datetime(n2, format='%Y')
		mk_yr = pd.date_range(y1,y2-timedelta(days=-380),freq='Y').year
	...
	variant = sorted(df['Variant_Clean'].loc[df['Model_Clean'] == Model_Clean].drop_duplicates().tolist())
	variant.insert(0,'Select Variant')
	Variant_Clean = col4.selectbox('Select Variant',variant,index = 0)
	
	fuel = sorted(df['Fuel_Clean'].loc[df['Variant_Clean'] == Variant_Clean].drop_duplicates().tolist())
	...
	cust_seg = sorted(df['SELLER_SEGMENT'].unique().tolist())
	cust_seg.insert(0,'Select Segmentation')
	SELLER_SEGMENT = col6.selectbox("Select Segmentation",cust_seg,index = 0)
	
	Meter_Reading = col7.number_input('Enter Meter Reading',min_value=None)
	
	state = sorted(df['CV_State_Clean'].unique().tolist())
	...
	loaded_model=pickle.load(open('cv_ncs.pkl','rb'))
	...
	state = sorted(df['STATE_MAPPED'].unique().tolist())
	STATE_MAPPED = col8.selectbox('Select State',state)

	try:
		X = pd.DataFrame(columns=['MAKE_YEAR','Make_Clean','Model_Clean','Variant_Clean','Fuel_Clean','STATE_MAPPED','Meter_Reading'],data=np.array([MAKE_YEAR,Make_Clean,Model_Clean,Variant_Clean,Fuel_Clean,STATE_MAPPED,Meter_Reading]).reshape(1,7))
		if st.button('Get the Best Price'):
			predicted_amount = loaded_model.predict(X)
			st.subheader(f"The Estimated CV Price is ₹ {predicted_amount[0]:.2f}")
	except Exception as e:
	...
	state = sorted(df['State_Clean'].loc[df['Make_Clean'] == Make_Clean].drop_duplicates().tolist())
	state.insert(0,'Select State')
	...
	MAKE_YEAR = col6.number_input('Enter Make Year in YYYY format', min_value=None, max_value=None, step=1) 


	cust_seg = sorted(df['Customer_Segmentation'].unique().tolist())
	cust_seg.insert(0,'Select Segmentation')
	...
	state = sorted(df['STATE_MAPPED'].loc[df['Make_Clean'] == Make_Clean].drop_duplicates().tolist())
	state.insert(0,'Select State')
	...
	cust_seg = sorted(df['SELLER_SEGMENT'].loc[df['Variant_Clean'] == Variant_Clean].drop_duplicates().tolist())
	cust_seg.insert(0,'Select Segmentation')
	...
	MAKE_YEAR = col1.number_input('Enter Make Year in YYYY format', min_value=None, max_value=None, step=1) 
	...
	def cleaning_text(final_result):

		state_code = ['AN','AP','AR','AS','BR','CH','CG','DN','DD','DL','GA','GJ','HR','HP','JH','JK','KA','KL','LD','MP','MH','MN','ML','MZ','NL','OR','OD','PY','PB','PN','RJ','SK','TN','TR','UP','UK','WB']

		if(len(final_result)==10):
			last_four_digit=final_result[-4:]
			to_remove_chars = {"O": "0", "I": "1", "G":"6","S": "5","Z": "7","A":"4","D":"0","E":"8"}#MPDSCFEZ44
			for char in to_remove_chars.keys():
				last_four_digit = last_four_digit.replace(char, to_remove_chars[char])

			# first two digit
			first_two_digit = final_result[0:2]
			to_remove_chars = {"1": "I", "5": "S", "6":"G", "7": "Z", "0": "O","4":"A"}
			for char in to_remove_chars.keys():
				first_two_digit = first_two_digit.replace(char, to_remove_chars[char])
			if(first_two_digit in state_code):
				if(first_two_digit =='DL'):
					third_fourth_digit = final_result[2:4]
					fivth_sixth_digit =  final_result[4:6]
					to_remove_chars = {"1": "I", "6":"G", "5": "S", "7": "Z", "0": "O","4":"A"}
					for char in to_remove_chars.keys():
						fivth_sixth_digit = fivth_sixth_digit.replace(char, to_remove_chars[char])

					complete_number_plate = first_two_digit + third_fourth_digit + fivth_sixth_digit + last_four_digit  
					return('if len = 10',complete_number_plate)
				
				else:
					third_fourth_digit = final_result[2:4]
					to_remove_chars = {"I": "1","G":"6", "S": "5", "Z": "7", "O": "0","A":"4","D":"0"}
					for char in to_remove_chars.keys():
						third_fourth_digit = third_fourth_digit.replace(char, to_remove_chars[char])

						# 5th and 6th digit improvement
					five_sixth_digit = final_result[4:6]
					to_remove_chars = {"1": "I", "6":"G", "5": "S", "7": "Z", "0": "O","4":"A"}
					for char in to_remove_chars.keys():
						five_sixth_digit = five_sixth_digit.replace(char, to_remove_chars[char])

					complete_number_plate = first_two_digit + third_fourth_digit + five_sixth_digit + last_four_digit
					return('len = 10 else',complete_number_plate)
			else:
					complete_number_plate = final_result
					return('else',complete_number_plate )


				

		# if number plate contain 11 digit        
		elif(len(final_result)==11):
		# first two digit
			first_two_digit = final_result[0:2]
			...
			last_four_digit=final_result1[-4:]
			to_remove_chars = {"O": "0", "I": "1", "G":"6","S": "5","Z": "7","A":"4","D":"0","E":"8"}#MPDSCFEZ44
			for char in to_remove_chars.keys():
				last_four_digit = last_four_digit.replace(char, to_remove_chars[char])

			# remaining digit
			first_two_digit = final_result1[0:2]        
			if(first_two_digit =='DL'):
				third_fourth_digit = final_result1[2:4]
				fivth_sixth_digit =  final_result1[4:6]
				to_remove_chars = {"1": "I", "6":"G", "5": "S", "7": "Z", "0": "O","4":"A"}
				for char in to_remove_chars.keys():
					fivth_sixth_digit = fivth_sixth_digit.replace(char, to_remove_chars[char])

				complete_number_plate = first_two_digit + third_fourth_digit + fivth_sixth_digit + last_four_digit  
				return('if len = 11 ',complete_number_plate)
			else:
				third_fourth_digit = final_result1[2:4]
				to_remove_chars = {"I": "1","G":"6", "S": "5", "Z": "7", "O": "0","A":"4","D":"0"}
				for char in to_remove_chars.keys():
				...
				# 5th and 6th digit improvement
				five_sixth_digit = final_result1[4:6]
				to_remove_chars = {"1": "I", "6":"G", "5": "S", "7": "Z", "0": "O","4":"A"}
				for char in to_remove_chars.keys():
					five_sixth_digit = five_sixth_digit.replace(char, to_remove_chars[char])

				complete_number_plate = first_two_digit + third_fourth_digit + five_sixth_digit + last_four_digit
				return('len = 11 else',complete_number_plate)
				
				
		# if number plate length is 9     
		elif(len(final_result)==9):
			last_four_digit=final_result[-4:]
			to_remove_chars = {"O": "0", "I": "1", "G":"6","S": "5","Z": "7","A":"4","D":"0","E":"8"}#MPDSCFEZ44
			for char in to_remove_chars.keys():
				last_four_digit = last_four_digit.replace(char, to_remove_chars[char])
			...
			if(first_two_digit in state_code):
				first_two_digit = final_result[0:2]        
				if(first_two_digit =='DL'):
					third_fourth_fivth_digit = final_result[2:5]
					complete_number_plate = first_two_digit + third_fourth_fivth_digit + last_four_digit  
					return('if len = 9',complete_number_plate)
				else:
					third_fourth_digit = final_result[2:4]
					to_remove_chars = {"I": "1","G":"6", "S": "5", "Z": "7", "O": "0","A":"4","D":"0"}
					for char in to_remove_chars.keys():
						third_fourth_digit = third_fourth_digit.replace(char, to_remove_chars[char])
						#5th improvement
					fiveth_digit = final_result[4:5]
					to_remove_chars = {"1": "I", "6":"G", "5": "S", "7": "Z", "0": "O","4":"A"}
					for char in to_remove_chars.keys():
						fivth_digit = fivth_digit.replace(char, to_remove_chars[char])

					complete_number_plate = first_two_digit + third_fourth_digit + fivth_digit + last_four_digit
					return('len = 9 else',complete_number_plate)
			else:
				complete_number_plate = final_result
				return('check first two digit',complete_number_plate)
			
			
		else:
			complete_number_plate = final_result
			return('else block',complete_number_plate)
		...
		for result in results:
			...
			try:
				...
				if len(final_result) < 5:
					img_contr_obj=ImageEnhance.Contrast(img)
					e_img=img_contr_obj.enhance(factor=8)
					text = pytesseract.image_to_string(e_img, config='-l eng --psm 6 --oem 1 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890')
					raw = re.sub("[^A-Z0-9]", "", text)
					final_result = re.sub(r'[\W_]+', '', raw)
					print(final_result)
					cleaning_text(final_result)
					return final_result
	...
	df = pd.read_csv('4w_cs_model.csv',encoding='latin-1')
	...
	cust_seg = sorted(df['SELLER SEGMENT'].unique().tolist())
	cust_seg.insert(0,'Select Segmentation')
	SELLER_SEGMENT = col6.selectbox("Select Segmentation",cust_seg,index = 0)       
